...
use super::frontmatter::{FrontmatterError, ScriptSource};

pub fn expand_manifest(content: &str) -> Result<String, FrontmatterError> {
            ...
            Some(other) => {
                ...
                if let Some(remainder) = other.strip_prefix("cargo,") {
                    return Err(FrontmatterError::new(
                        format!(
                            "unsupported frontmatter infostring attributes: \
                             `{remainder}`"
                        ),
                        info_span,
                    )
                    ...
                    return Err(FrontmatterError::new(
                        format!(
                            "unsupported frontmatter infostring `{other}`; \
                             specify `cargo` for embedding a manifest"
                        ),
                        info_span,
                    )
                    .push_visible_span(close_span));
