...
/// Source:
/// https://github.com/rust-lang/cargo/blob/0f14d9d2f/src/cargo/util/frontmatter.rs
...
impl<'s> ScriptSource<'s> {
    pub fn parse(raw: &'s str) -> Result<Self, FrontmatterError> {
        use winnow::stream::{
            FindSlice as _, Location as _, Offset as _, Stream as _,
        };
        ...
        match fence_length {
            ...
            1 | 2 => {
                ...
                return Err(FrontmatterError::new(
                    format!(
                        "found {fence_length} `{FENCE_CHAR}` in rust \
                         frontmatter, expected at least 3"
                    ),
                    raw.len()..raw.len(),
                )
        ...
        let Some(frontmatter_nl) = input.find_slice(nl_fence_pattern.as_str())
        else {
            for len in (2..(nl_fence_pattern.len() - 1)).rev() {
                let Some(frontmatter_nl) =
                    input.find_slice(&nl_fence_pattern[0..len])
                else {
                    continue;
                };
                ...
                return Err(FrontmatterError::new(
                    format!(
                        "closing code fence has {fewer_dashes} less `-` than \
                         the opening fence"
                    ),
        ...
        if 0 < extra_dashes {
            ...
            return Err(FrontmatterError::new(
                format!(
                    "closing code fence has {extra_dashes} more `-` than the \
                     opening fence"
                ),
            ...
            let after_closing_fence =
                after_closing_fence.trim_matches(is_horizontal_whitespace);
